...
def decrypt_with_huffman(input_file, output_file, huffman_codes):
    with open(input_file, 'rb') as f:
        padding_size = ord(f.read(1))

        byte_data = f.read()
        encoded_text = ''.join(format(byte, '08b') for byte in byte_data)


    encoded_text = encoded_text[:-padding_size]

    decoded_text = decode_with_huffman(encoded_text, huffman_codes)
...
decrypt_with_huffman(output_file, output_file_decrypted, huffman_codes)
